...
bool Game::initialize() {
    ...
    d_renderer = SDL_CreateRenderer(
        d_window, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
...
void Game::updateGame() {
    ...
    d_paddlePos.y =
        min(d_paddlePos.y, HEIGHT - THICKNESS - PADDLE_HEIGHT / 2.0);
    ...
    if (d_ballPos.x <= THICKNESS + BALL_WIDTH / 2.0) {
        if (d_paddlePos.y - PADDLE_HEIGHT / 2.0 <= d_ballPos.y &&
            d_paddlePos.y + PADDLE_HEIGHT / 2.0 >= d_ballPos.y) {
            d_ballDirection.x *= -1;
        } else {
            SDL_Log("Lost");
        }
...
void Game::generateOutput() {
    ...
    // Draw Ball
    {
        SDL_Rect ball{static_cast<int>(d_ballPos.x - BALL_WIDTH / 2.0),
